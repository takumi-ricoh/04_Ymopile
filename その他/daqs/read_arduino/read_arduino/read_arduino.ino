...
//　 センサのアドレスは、0x00からインクリメント
//
//   センサには、0x01からアドレスを振っておくこと。
//   1chあたりの測定時間は、2[ms]。127個読むのには約254[ms]要する。
//   センサのデータ更新時間が100[ms]のため、同じセンサを読み出すのは100[ms]以上経過後にする処理を入れてある。
//   どのセンサの出力値かわかるように、データの前にセンサ番号を追加。

#include <Wire.h>

#define WAIT 100            //Wait時間（同一センサを読む時の間隔が、100[ms]以上になるようにする）
#define SENSOR_NUM 24       //センサ数
#define SENSOR_BASEADR 0x5A-1 //センサのベースアドレス
  ...
  if (dt > WAIT) {        //一つ目のセンサを読むタイミングが、前回測定時より間隔が、WAIT[ms]以上になら測定開始
    ...
    for ( i = 1; i < SENSOR_NUM+1; i++ ) {          
      ...
      if( Tobj > 0.0 ) {                          // 接続されているセンサの結果だけ出力（接続されていないセンサからの戻り値は0[deg]）
        ...
        Serial.print("0,0,");                     //sensor data // reserved
        Serial.print( Tobj );                    // obj temp
        Serial.print( "," );                     // 結果を出力する
        Serial.print( Tamb );                    // sensor temp
